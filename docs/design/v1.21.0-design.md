...
Policy-related fields are included **only when policy resolution has occurred**.
Snapshots generated without policy input will omit the `policy` block entirely.
