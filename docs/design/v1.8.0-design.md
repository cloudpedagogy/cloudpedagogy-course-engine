...
Explain JSON outputs are deterministic with the exception of runtime metadata.

- `engine.built_at_utc` is included to support audit and traceability
- All other fields must be stable given identical inputs
- File ordering, key ordering, and path normalisation are deterministic
- Hashes are used as the canonical signal for content change

This balance supports governance and audit use cases without undermining reproducibility.
